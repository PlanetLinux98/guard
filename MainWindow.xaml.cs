...
public sealed partial class MainWindow : Window
{
    ...
    private async void OnExportAppSettings(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                _appStatusText = "No settings folders matched the ticked apps. Apps that keep settings in the registry, in ProgramData, or under a folder name unlike the app name are not found by this search.";
                AnnounceAppStatus();
                return;
            }
            ...
            if (await ShowDialogAsync(dlg) != ContentDialogResult.Primary)
            {
                _appStatusText = "App settings export cancelled. Nothing was copied.";
                AnnounceAppStatus();
                return;
            }
            ...
            if (chosen.Count == 0)
            {
                _appStatusText = "No folders were ticked. Nothing was copied.";
                AnnounceAppStatus();
                return;
            }
        ...
        {
            _appStatusText = "App settings export failed: " + ex.Message;
            AnnounceAppStatus();
        }
        finally
        {
