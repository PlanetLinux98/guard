...
// Confirmation step for Reinstall & Restore Settings: restore overwrites real
// per-user data, so nothing is touched until the user has seen each target path
// (and its "Replaces existing" warning) and could untick it. Sizes come from the
// export manifest, so unlike the export dialog there's no background measuring.
